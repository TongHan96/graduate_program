...
switch type
    case 'homonorm'
        ...
        ar_mat = 1*toeplitz(0.5.^(0:p-1));
        % ar_mat = 1*toeplitz(0.5.^(0:p-1))/min(n^(1/7),p^(1/5)); % p*AR(1) covariance matrix, phi=0.5, make the eigenvalue smaller
    ...
    case 'homonorm2'
        rng(1); % to fix B and mu
        ar_mat = 1*toeplitz(0.5.^(0:p-1))/log10(p);
        % ar_mat = 1*toeplitz(0.5.^(0:p-1))/min(n^(1/7),p^(1/5)); % p*AR(1) covariance matrix, phi=0.5, make the eigenvalue smaller
        Z =  mvnrnd(zeros(1,p), ar_mat, n);
        [Zdecomp,~] = eig(Z*Z');
        % Zdecomp' * Zdecomp
    ...
    case 'heternorm'
        ...
        % ar_mat = 1*toeplitz(0.5.^(0:p-1))/min(n^(1/7),p^(1/5));
        Z =  mvnrnd(zeros(1,p), ar_mat, n);
        [Zdecomp,~] = eig(Z*Z');
        % Zdecomp' * Zdecomp
    ...
    case 'heternorm2'
        rng(1);
        ar_mat = 1*toeplitz(0.5.^(0:p-1))/log10(p); % p*AR(1) covariance matrix
        % ar_mat = 1*toeplitz(0.5.^(0:p-1))/min(n^(1/7),p^(1/5));
        Z =  mvnrnd(zeros(1,p), ar_mat, n);
        [Zdecomp,~] = eig(Z*Z');
        % Zdecomp' * Zdecomp
    ...
    case 'pois2'
        rng(1); % since B is a fixed matrix.
        ar_mat = cov_mat(ones(p,1)*sqrt(4), 0.5)/log10(p);
        % ar_mat = cov_mat(ones(p,1)*sqrt(4), 0.5)/min(n^(1/7),p^(1/5)); % p*AR(1) covariance matrix
        Z =  mvnrnd(zeros(1,p), ar_mat, n);
        [Zdecomp,~] = eig(Z*Z');
        B = sqrt(1/n)*Z'* Zdecomp(:,end:-1:end-q+1); % sort the eigenvectors by decreasing eigen values.
        sB = sign(B(1,:));
    ...
    case 'pois_bino2'
        rng(1); % since B is a fixed matrix.
        ar_mat = cov_mat(ones(p,1)*sqrt(6), 0.5)/log10(p);
        % ar_mat = cov_mat(ones(p,1)*sqrt(6), 0.5)/min(n^(1/7),p^(1/5)); % p*AR(1) covariance matrix
        Z =  mvnrnd(zeros(1,p), ar_mat, n);
        [Zdecomp,~] = eig(Z*Z');
        B = sqrt(1/n)*Z'* Zdecomp(:,end:-1:end-q+1); % sort the eigenvectors by decreasing eigen values.
        sB = sign(B(1,:));
    ...
    case 'npb2'
        ...
        Z = 1/3*randn(p,q);
        [B0tmp, ~] = qr(Z, 0);
        % B0= B0tmp * diag(sort(sqrt(eig(Z'*Z)), 'descend'))/sqrt(min(n^(1/7),p^(1/5)));
        B0= B0tmp * diag(sort(sqrt(eig(Z'*Z)), 'descend'))/sqrt(log10(p));
        sB = sign(B0(1,:));
        B = B0.*repmat(sB,p,1); % ensure B first nonzero is positive
        mu = 0.4*randn(1,p); % Intercept
        % sB = sign(B(1,:));
